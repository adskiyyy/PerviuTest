...
class VK_Sender(config.Settings):
    ...
    def get_groups_user(self, api):
        with open('C:/Users/ЛУПСИК/Desktop/pet-project/Pervonah-master/groups.txt', 'r') as f:
            groups_ids = [line.strip() for line in f.readlines()]
        return groups_ids


    def vk_send_messages(self, api):
        self.get_groups_user = self.get_groups_user
        while True:
            try:
                time.sleep(self.config.DELAY)

                post = api.wall.get(filters='owner',counts=1,domain=random.choice(self.get_groups_user))
                if len(post['items']) == 0:
                    continue
